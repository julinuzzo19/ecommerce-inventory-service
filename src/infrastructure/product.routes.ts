...
const createProductController = () => {
  const inventoryRepository = new InventoryTypeORMRepository(PostgresDataSource.manager);

  const unitOfWork = new UnitOfWorkTypeORM(PostgresDataSource);

  const createInventoryProductCommand = new CreateInventoryProductCommand(unitOfWork, inventoryRepository);

  const getProductInventoryUseCase = new GetProductInventoryUseCase(inventoryRepository);

  const getStockAvailableOrderUseCase = new GetStockAvailableOrderUseCase(inventoryRepository);
